...
def test_pacing_hint_empty():
    state = PacingState(current_cards=5, covered_slides=[1, 2], total_pages=10, focus_prompt="", target_density=1.5)
    assert state.hint == ""  # Not enough cards to give feedback

def test_pacing_hint_normal_mode_balanced():
    state = PacingState(current_cards=15, covered_slides=[10], total_pages=20, focus_prompt="", target_density=1.5)
    hint = state.hint
...
def test_pacing_hint_normal_too_dense():
    state = PacingState(current_cards=30, covered_slides=[10], total_pages=20, focus_prompt="", target_density=1.5)
    hint = state.hint

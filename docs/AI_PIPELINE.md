...
Lectern uses Google Gemini 3 Flash as the core engine for multimodal flashcard generation. The V2-only pipeline is orchestrated by `GenerationAppServiceImpl` (`lectern/application/generation_app_service.py`) with infrastructure adapters in `lectern/infrastructure/`.
...
- **Pacing System:** Managed by `ai_pacing.py`. Based on the character density per page, the app switches between **Script** (>=400 chars/page), **Normal**, and **Slides** mode to adjust the token budget and card target density dynamically.

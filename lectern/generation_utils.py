...
def _coverage_is_sufficient(coverage_data: CoverageData) -> bool:
    ...
    return high_priority_ok and relation_ok and (
        explicit_concept_pct >= 60 or page_pct >= 75
    )
...
class CardPriorityScorer:
    def __init__(self, weights: ReflectionScoringWeights | None = None) -> None:
...
def _select_best_reflection_cards(
    *,
    ...
    return selected, {
        ...
        "concept_coverage_delta": int(selected_coverage.get("explicit_concept_count", 0))
...
def collect_card_fronts(cards: list[CardData]) -> list[str]:
    return [_get_card_front(card) for card in cards if _get_card_front(card)]

...
### Fixed
- **Testing**: Fixed outdated text assertion (`"Quality & Trust"` to `"Generation Health"`) in the E2E test suite to unblock the release CI pipeline.

## [1.15.1] - 2026-03-29

### Fixed
- **Build**: Fixed TypeScript ESLint errors caused by unused variables in `ProgressView.tsx` which blocked the 1.15.0 CI pipeline.
...
### Added
- **CI Quality Gates**: Expanded GitHub Actions workflow with linting and end-to-end test coverage, including integrated smoke tests.

### Changed
- **API Versioning**: Backend now reports the application version from `lectern.version.__version__` for consistent version surfacing across the stack.

### Fixed
- **Anki Connectivity**: Improved Anki connection handling and logging for more robust diagnostics.

        ...
        <div className="h-14 px-6 border-b border-border flex items-center justify-between bg-surface/30 backdrop-blur-sm shrink-0 rounded-tr-2xl">
                ...
                <div className="flex items-center gap-2 text-text-main font-semibold text-sm">
                    ...
                    {isHistorical ? (
                        <span className="flex items-center gap-2">
                            Archive View
                            <span className="px-2 py-0.5 bg-primary/10 rounded text-[10px] font-mono text-primary">HISTORICAL</span>
                        </span>
                    ) : (
                ...
                <div className="flex gap-1 text-[10px] font-bold">
                    {(['creation', 'topic', 'slide', 'type'] as const).map((opt) => (
                        <button
                            ...
                            className={clsx(
                                "px-3 py-1 uppercase tracking-wider rounded-md transition-colors",
                                sortBy === opt
            ...
            <div className="flex items-center gap-3">
                ...
                {step === 'done' && onFocusMode && (
                    <button
                        onClick={onFocusMode}
                        className="flex items-center gap-1.5 px-2.5 py-1.5 bg-surface hover:bg-surface/80 text-text-muted hover:text-primary rounded-lg text-xs font-medium transition-colors"
                ...
                <div className="relative group">
                    ...
                    <input
                        ...
                        placeholder="Search..."
                        className="pl-8 pr-3 py-1.5 text-xs bg-surface/50 border-0 rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/50 w-36 focus:w-52 transition-all duration-300 placeholder:text-text-muted/50"
                    />
                </div>
                <div className="flex items-center px-2 py-1 bg-surface/50 rounded text-[10px] font-mono text-text-muted">
